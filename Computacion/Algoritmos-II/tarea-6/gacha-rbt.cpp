...
struct Node {
    ...
    explicit Node(string val) : data(val), color(RED), left(nullptr), right(nullptr), parent(nullptr) {}
};
...
public:
    ...
    // search for a value in the tree (helper)
    Node* searchHelper(Node* node, string val) {
        if (node == nullptr)
            return nullptr;
